...
function render(todoData) {
    ...
    $(document).ready(function(){
        ...
        $("#user p").text(myUser.name);
        $("#user img").attr("src",myUser.picture);
...
$("#addTodoBtn").on("click", function () {
    ...
    database.ref(`tododata_Firebase/${myUser.id}`).push(newTodoData).then(function (snapshot) {
...
// database.ref(`tododata_Firebase/${myUser.id}`).once("value",function(snapshot){
// database.ref(`tododata_Firebase`).once("value", function (snapshot) {
...
// var newDesign = design.map(function(current){	
// if(current=="Estela"){
//     return current + "好可愛"; 
//     } else {
//         return current
